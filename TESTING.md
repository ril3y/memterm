...
- `MEMTERM_CLAUDE_DIR` (stage 3) points the ONE core ~/.claude parsing site (`Adapters.defaultClaudeProjectsDir`/`defaultClaudeSessionsDir`) at a fixture tree (`<dir>/projects`, `<dir>/sessions`). The `claude-*` probe legs build that tree under the isolated `MEMTERM_STATE_DIR` and set the variable in-process, so the browser/badge legs never involve the founder's real `~/.claude`; the read-only real-data check is observe mode's `observe-claude-real` leg (list-only, invalid-id-free, mtimes asserted unchanged on the scanned cold project).
...
- **Semantics update (founder-amended FR-56, approved 2026-09-02, schema v6):** the `save-close-forget` leg is the one place a gate's MEANING legitimately changed — a user close now ARCHIVES instead of deleting. The leg asserts archived-not-restored: run 1 asserts the closed tab's `sessions` row (`close_reason=user-close`), its frozen files under `<state>/archive/<rowid>/`, and clean live tables (`SMOKE-ARCHIVED`); run 2 keeps the original NOT-restored assertion unchanged and additionally asserts the archive row + frozen scrollback survived the crash-sim relaunch, launch orphan sweep, and retention pass (`SMOKE-ARCHIVE-SURVIVED`). Explicit Forget gestures remain true deletion — covered by the probe legs `archive-on-close` and `forget-everything-empties-archive` (which runs LAST by design: it wipes the isolated probe state dir) and by the `ArchiveTests` forget-scope matrix.
...
6. Run everything **against the dist binary**: `--config-dump`; `--bench`/`--latency`/`--flood` with enforced thresholds; `--smoke=save` + `--smoke=verify` in a fresh `mktemp` dir; `MEMTERM_UI_PROBE=1` in `fresh`, `restored` (seeded from a smoke save), and `observe` modes; the config matrix (quiet) + one visible pixel pass; serial coverage via the probe's pty-pair legs (real `/dev/cu.*` nodes are never touched; `MEMTERM_SERIAL_PROBE_PTY` remains the connect-sheet stand-in hook); the claude-browser extension legs (`claude-fixtures`/`claude-browser-panel`/`claude-browser-resume`/`claude-browser-badges`, stateful — fresh AND restored) over a `MEMTERM_CLAUDE_DIR` fixture tree, plus observe mode's read-only `observe-claude-real` against the real `~/.claude`; the timeline extension legs (`timeline-panel`/`timeline-search`/`timeline-reopen-ghost`/`timeline-resume`/`timeline-forget-card`/`timeline-parked-workspace`, stateful — fresh AND restored), which run between `archive-on-close` (their fixture: the archived tab plus a `.hist` FTS needle) and `forget-everything-empties-archive` (still LAST by design).
