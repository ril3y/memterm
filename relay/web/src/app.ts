...
let pendingEphemeral: CryptoKey | undefined;
...
async function startSession(): Promise<void> {
  ...
  pendingEphemeral = ephemeralPrivate;
...
async function handleEnvelope(from: string, payload: Uint8Array): Promise<void> {
  ...
  if (!session) {
    ...
    const ephemeral = pendingEphemeral;
    pendingEphemeral = undefined;
    const keys = await deriveKeys(ephemeral, hostEphemeralRaw, hostIdForKeys, identity.id, false);
