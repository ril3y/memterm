...
- **FR-56 (M)** — *Deliberate close forgets (founder decision 2026-08-30):* memory follows intent. A **user-initiated** close (⌘W on a pane/tab, closing a window, "Forget" on a workspace) removes those panes from the restore set at the moment of the gesture — they must NOT reopen on the next launch, after a reboot, or ever. Only **abnormal** endings (crash, force-quit, kernel panic, reboot with memterm open) restore everything that was open. Closed items may move to the session timeline (FR-43) as history, but never back into a window uninvited. Corollary: quitting the app normally (⌘Q) preserves the then-open set — quit is "put it down," close is "throw it away."
