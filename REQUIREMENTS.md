...
- **FR-56a (M)** — *A close never deletes a workspace (founder decision 2026-09-10, supersedes the 2026-08-31 stage-2 auto-remove):* a user close that empties a non-Default workspace **parks** it — chip kept as "(parked)", name and color kept, reopenable from the chip, the switcher, or its timeline card — and its tabs archive as any user close does. Only an explicit Forget deletes a workspace. Rationale: the red close button closes every tab in a window, and under the auto-remove rule a run of window closes deleted six workspaces in twenty seconds (archive evidence). Corollary: memterm runs as a **single instance** per state directory (a second launch of the packaged app activates the running one), because two instances race each other's scoped topology saves.
