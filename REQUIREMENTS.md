...
- **FR-59 (M)** — *Switching is non-destructive (founder decision 2026-08-31, supersedes FR-51's switch mechanics):* switching workspaces HIDES the outgoing workspace's windows (orderOut) and keeps every pane's pty and process alive — a running `ping`, build, or Claude session continues untouched and is exactly as it was on switch-back, with no "restored" divider and no re-spawn. The journal→resurrect pipeline (ghost scrollback + consent-gated offers) runs ONLY when the processes are actually gone: app launch after quit/crash/reboot, and reopening a PARKED workspace. **Park** remains the explicit "close but keep memory" gesture — parking may terminate processes; switching never does. Capture continues for hidden workspaces (their panes are still live). Corollary: closing the last visible tab while other workspaces are hidden must surface a hidden workspace or a fresh window, never quit the app or strand it windowless. *Frame stability (founder, same day):* windows never jump — a switch shows the same window objects exactly where they were, and the resurrect path (launch/unpark) places windows at their journaled frames without centering or cascading; only a frame that would land off-screen gets adjusted.
- **FR-60 (M)** — *Remote attach (founder decision 2026-09-19):* off by default; a blind relay that never decrypts traffic and persists nothing; the host is the running app itself, with no daemon or launch agent. A paired remote client can do only what a local keyboard can do to a pane — see it, type into it, resize it, open tabs — nothing more. Crypto: P-256 identity keys plus per-session ephemeral keys, HKDF-SHA256 with info `"memterm-remote-v1" ‖ hostId ‖ deviceId`, AES-256-GCM with 96-bit per-direction counter nonces; host ID = base32(SHA-256(host public key))[0..<16], lower case, unpadded. Per-viewer output backlog is capped at 256 KB, oldest bytes dropped first. A pairing token is single-use with a 2-minute expiry. Unlock attempts are rate-limited to 5 per minute per device. Deferred: workspace locks (protocol hooks already exist for them) and the SSH host manager (its own spec) are out of scope here. Config keys: `[remote] enabled`, `relay_url`.
- **FR-56 (M)** — *Deliberate close archives (founder decision 2026-08-30, **amended 2026-09-02**):* memory follows intent. A **user-initiated** close (⌘W on a pane/tab, closing a window) removes those panes from the restore set at the moment of the gesture — they must NOT reopen on the next launch, after a reboot, or ever — but their scrollback and per-tab history now move into the **timeline archive** (schema v6 `sessions` + FTS, frozen files under `<state>/archive/`) instead of being deleted: searchable, visible immediately in the timeline, per-card Forget available. **Explicit Forget gestures** (Forget Pane/Tab/Workspace Memory, per-card Forget, Forget Everything) remain TRUE deletion — archive included. Only **abnormal** endings (crash, force-quit, kernel panic, reboot with memterm open) restore everything that was open. Archived items never come back into a window uninvited. Corollary: quitting the app normally (⌘Q) preserves the then-open set — quit is "put it down," close is "file it away," explicit Forget is "burn it." Retention is configurable (`archive_retention_days`, Settings ▸ Memory; default 90, 0 = forever); NFR-10 perms extend to the archive dir.
- **FR-56a (M)** — *A close never deletes a workspace (founder decision 2026-09-10, supersedes the 2026-08-31 stage-2 auto-remove):* a user close that empties a non-Default workspace **parks** it — chip kept as "(parked)", name and color kept, reopenable from the chip's right-click menu (a plain click on a parked chip does nothing — founder 2026-09-21), the switcher, or its timeline card — and its tabs archive as any user close does. Only an explicit Forget deletes a workspace. Rationale: the red close button closes every tab in a window, and under the auto-remove rule a run of window closes deleted six workspaces in twenty seconds (archive evidence). Corollary: memterm runs as a **single instance** per state directory (a second launch of the packaged app activates the running one), because two instances race each other's scoped topology saves.
- **FR-57 (M)** — *Kill memories:* explicit forget commands at every granularity — "Forget Pane Memory" (drop journal rows + scrollback file for one pane), "Forget Workspace…" (FR-51's forget path), and the global "Forget Everything" (FR-45). Forgetting deletes the scrollback files from disk, not just the index rows.
- **FR-58 (M)** — *Right-click is a first-class affordance:* context menus everywhere the mouse already is — on a tab: Move to Workspace ▸, New Workspace from Tab, Rename, Forget Tab Memory, Close; on the workspace titlebar chip: the full workspace switcher/create/park/forget menu; in a pane: Copy/Paste, Split Right/Down, Clear, Forget Pane Memory. Creating a workspace must be reachable by right-click alone, never only via menu bar or keyboard.
